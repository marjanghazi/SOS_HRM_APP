      ...
      switch ($this->dbType()) {
        case 'mysql':
          ...
          if (PHP_VERSION_ID >= 80500 && class_exists('PDO\Mysql')) {
            $attr = PDO\Mysql::ATTR_USE_BUFFERED_QUERY;
          }
          $pdo->setAttribute($attr, $is_buffered);
          break;

        case 'pgsql':
          $attr = PDO::PGSQL_ATTR_DISABLE_PREPARES;
          if (PHP_VERSION_ID >= 80500 && class_exists('PDO\Pgsql')) {
            $attr = PDO\Pgsql::ATTR_DISABLE_PREPARES;
          }
          $pdo->setAttribute($attr, 1);
          break;
      }
